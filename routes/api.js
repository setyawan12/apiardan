...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya api chat author...'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di wa.me/6285156482636 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
	...
	if (!username) return res.json(loghandler.notusername)
       fetch(encodeURI(`https://zahirr-web.herokuapp.com/api/ig/stalk?username=${username}&apikey=zahirgans`))
		...
		var username = data.result.Username;
             res.json({ status : true, creator : `${creator}`, Result : { Biodata : `${biodata}`, follower : `${follower}`, Following : `${following}`, Jumlah_post : `${post}`, nama : `${nama}`, profil_pic : `${profil}`, username : `${username}`}})    
         })
         .catch(e => {
         	res.json(loghandler.error)
})
...
router.get('/googleimg', async (req, res, next) => {
	...
	if(!search) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter search"})
       fetch(encodeURI(`https://api.zeks.me/api/gimg?apikey=ardanfajars&q=${search}`))
        .then(response => response.json())
        .then(data => {
...
router.get('/random/citacita', async (req, res, next) => {
        var apikeyInput = req.query.apikey
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'free') return res.json(loghandler.invalidKey)

       fetch(encodeURI(`https://raw.githubusercontent.com/setyawan12/afs-rest/main/citacita.json`))
        .then(response => response.json())
        .then(data => {
        	nimekkk =  data[Math.floor(Math.random() * data.length)];
        var result = nimekkk;
             res.json({
             	creator: `${creator}`,
                 result,
